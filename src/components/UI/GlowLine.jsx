const COLOR_SCHEMES = {
  ...
  fire: {
    core: "via-orange-500",
    glow: ["via-orange-500", "via-red-600", "via-amber-500", "via-orange-400"],
  },
...
const GlowLine = ({
  ...
  const selectedScheme = COLOR_SCHEMES[color] || COLOR_SCHEMES.amber;
  ...
  return (
    <div className={`${containerClasses} ${className}`} style={positionStyle}>
      ...
      />
      <div
        className={`absolute inset-0 ${isVertical ? "w-0.5 -ml-px" : "h-0.5 -mt-px"} ${gradientDirection}
          from-transparent via-white to-transparent opacity-60`}
      />
      {glowLayers.map((layer, index) => (
        <div
          key={index}
          className={`absolute inset-0 ${layer.size} ${gradientDirection} from-transparent ${layer.color}
            to-transparent ${layer.blur} ${layer.opacity}`}
        />
      ))}
    </div>
  );
